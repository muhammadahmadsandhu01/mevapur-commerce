...
export default function ProductCard({ product }: { product: Product }) {
  ...
  const targetHref = `/products/${product.slug || product._id}`;
  ...
  return (
    <article className="group relative flex h-full flex-col border border-slate-200 bg-white rounded-xl overflow-hidden transition hover:border-slate-300 hover:shadow-md">
      <Link href={targetHref} className="absolute inset-0 z-0" aria-label={`View ${product.name}`} />

      <div className="relative z-10 aspect-square overflow-hidden bg-slate-100">
        <Image
          src={image}
          alt={product.name}
          fill
        ...
        <button
          ...
          className="absolute right-2 top-2 rounded-full bg-white/95 backdrop-blur p-2 text-slate-800 shadow-sm transition hover:text-[#ff8a00] focus:outline-none focus:ring-2 focus:ring-[#ff8a00] disabled:opacity-50"
        >
