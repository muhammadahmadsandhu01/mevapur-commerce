 ...
 *
 * PROVENANCE METADATA:
 * - Country Identity: ISO 3166-1:2020 (ISO 3166 Maintenance Agency)
 * - Country Calling Codes: ITU-T E.164 (International Telecommunication Union)
 * - Address & Postal Requirement Policies: Universal Postal Union (UPU) / MevaPur Commerce Baseline v1.0
 * - Canonical Registry: backend/modules/commerce/registries/countryRegistry.js
...
const RAW_COUNTRIES: Array<[string, string, string, string, string, string, string, CountryPolicy['postalPolicy'], CountryPolicy['adminPolicy'], CountryPolicy['adminType']]> = [
...
for (const [code, alpha3, numeric3, name, defaultCurrency, defaultLocale, callingCode, postalPolicy, adminPolicy, adminType] of RAW_COUNTRIES) {
  COUNTRY_POLICIES[code] = {
    code,
    alpha3,
    numeric3,
    name,
