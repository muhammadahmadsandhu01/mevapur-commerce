      ...
      try {
        ...
        if (response.data.success) {
          ...
          const formValues: ProductFormData = {
            ...
            allowWishlist: product.allowWishlist !== false,
    ...
    <div>
      ...
      <div style={{ marginBottom: '32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          ...
          >
            {loading || saving ? <Loader size={18} style={{ animation: 'spin 1s linear infinite' }} /> : <CheckCircle size={18} />}
            {loading || saving ? 'Saving...' : 'Update Product'}
          </button>
        </div>
      </div>
          ...
          {formData.productType === 'variable' && (
            <Section title="Product Variants" icon={Package} defaultOpen={false}>
              ...
              {formData.variants.map((variant, index) => (
                <div key={variant._id || index} style={{
                  padding: '20px',
                  border: '1px solid var(--border-color)',
                  borderRadius: '8px',
                  marginBottom: '16px',
