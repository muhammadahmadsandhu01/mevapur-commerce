  ...
  if (!effectiveUri) {
    try {
      ...
      try {
        ...
        MongoMemoryServer = require(path.resolve(__dirname, '../../backend/node_modules/mongodb-memory-server')).MongoMemoryServer;
      }
      embeddedServer = await MongoMemoryServer.create();
      effectiveUri = embeddedServer.getUri() + 'mevapur_rollback_test';
    } catch {
      effectiveUri = 'mongodb://127.0.0.1:27017/mevapur_rollback_test';
    }
  }
