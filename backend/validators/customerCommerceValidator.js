...
const returnRequestSchema = z.object({ orderId: orderReference, items: z.array(z.object({ productId: objectId, quantity: z.number().int().min(1).max(20), reason: z.enum(['damaged', 'wrong_item', 'not_as_described', 'not_satisfied', 'duplicate', 'other']), reasonDetails: optionalText(500), condition: z.enum(['new', 'used', 'damaged']).optional() }).strict()).min(1).max(50), refundMethod: z.enum(['original_payment', 'store_credit', 'bank_transfer']).optional(), customerNotes: optionalText(1000) }).strict();

module.exports = { pagination, profileSchema, addressBody, addressUpdateSchema, idParam, productParam, reviewSubmitSchema, reviewUpdateSchema, returnRequestSchema };
