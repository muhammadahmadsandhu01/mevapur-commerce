...
exports.listDocuments = async (req, res, next) => { try { return success(res, 200, await service.listDocuments(req.user.id, req.params.id), req.requestId); } catch (error) { return next(error); } };
exports.getDocument = async (req, res, next) => { try { return success(res, 200, { document: await service.getDocument(req.user.id, req.params.id, req.params.documentNumber) }, req.requestId); } catch (error) { return next(error); } };
exports.getRecoveryState = async (req, res, next) => { try { return success(res, 200, { recovery: await service.getRecoveryState(req.user.id, req.params.id) }, req.requestId); } catch (error) { return next(error); } };
exports.retryPayment = async (req, res, next) => { try { return success(res, 200, { retry: await service.retryPayment(req.user.id, req.params.id, req.body) }, req.requestId); } catch (error) { return next(error); } };
