...
describe('Phase 3 — Concurrency, Idempotency & Durability Integration', () => {
  ...
  describe('2. Duplicate Simultaneous Requests & Race Deduplication', () => {
  ...
  describe('4. Strict UUID Format Validation for operationKey', () => {
  ...
  describe('5. Variant Ownership and Isolation Verification', () => {
    it('rejects variant ID that does not belong to the target product (404)', async () => {
      // Pass variant1Id (which belongs to variableProduct) to a DIFFERENT product or random ID
  ...
  describe('6. Idempotency Durability Survives Process/Memory Reset', () => {
    it('replays existing transaction without duplicating stock after fresh query', async () => {
      const opKey = crypto.randomUUID();

      // 1. Initial execution (+15) -> 50 + 15 = 65
      const res1 = await InventoryService.adjustStock({
        productId: String(simpleProduct._id),
        type: 'in',
        quantity: 15,
        reason: 'Initial adjustment before memory clear',
        operationKey: opKey,
        actorId: adminUser._id
      });

      expect(res1.product.newStock).toBe(65);
      expect(res1.idempotentReplay).toBe(false);

      // 2. Replay call using same key
      const res2 = await InventoryService.adjustStock({
        productId: String(simpleProduct._id),
        type: 'in',
        quantity: 15,
        reason: 'Replay adjustment after memory clear',
        operationKey: opKey,
        actorId: adminUser._id
      });

      expect(res2.product.newStock).toBe(65);
      expect(res2.idempotentReplay).toBe(true);

      const reloaded = await Product.findById(simpleProduct._id);
      expect(reloaded.stock).toBe(65);
    });
  });
});
